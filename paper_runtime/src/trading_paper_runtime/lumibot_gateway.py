    ...
    def _order_to_snapshot(self, order: object) -> OrderSnapshotPayload:
        ...
        * nothing asserted the mapped status was in the shared vocabulary.
        ...
        return OrderSnapshotPayload(
            ...
            # Milestone 11 follow-up: the "this runtime only submits DAY
            # LIMIT orders" justification for defaulting an absent
            # `time_in_force` to DAY only holds for an order this runtime
            # itself created. `list_open_orders`/`list_recent_orders` are
            # account-wide broker reads — they can return an order placed
            # manually or by another application against the same paper
            # account, for which no such assumption is safe. The default is
            # now applied only when the order's own `client_order_id` proves
            # it was generated by this project's own id schemes; otherwise
            # an absent time_in_force fails closed like any other malformed
            # field. A *present but unrecognized* value always fails closed
            # regardless of ownership.
            time_in_force=normalize_time_in_force(
                _time_in_force_with_ownership_default(
                    getattr(raw_tif, "value", raw_tif) if raw_tif is not None else None,
...
def _is_runtime_owned_client_order_id(client_order_id: str) -> bool:
    """True only if `client_order_id` was generated by this project's own
    order-identity schemes: `"intent-"` for the in-process ADR 0001 adapter
    (`runtime/client/models.py::derive_client_order_id`) or `"epb-"` for
    external paper-book orders (`external_broker.py::
    derive_external_order_identity`). Anything else may be a manually
    placed order, or one placed by an unrelated application against the
    same paper account, and must not be assumed to share this runtime's own
    submission defaults."""
    return client_order_id.startswith("intent-") or client_order_id.startswith("epb-")


def _time_in_force_with_ownership_default(raw_time_in_force: str | None, client_order_id: str) -> str:
    """Milestone 11 follow-up: only default an absent broker
    `time_in_force` to `"DAY"` when the order's own `client_order_id`
    proves this runtime created it — `list_open_orders`/`list_recent_orders`
    are account-wide reads that can surface orders this runtime never
    submitted, for which no default is justified."""
    if raw_time_in_force is not None:
        return raw_time_in_force
    if _is_runtime_owned_client_order_id(client_order_id):
        return "DAY"
    raise NormalizationError(
        f"broker order time_in_force is absent for client_order_id {client_order_id!r}, which is outside "
        "this runtime's own client_order_id namespace — a DAY default cannot be assumed for an order "
        "this runtime may not have created"
    )
