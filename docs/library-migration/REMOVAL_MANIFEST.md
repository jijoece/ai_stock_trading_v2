...
**PR 3 update:** the custom market-calendar row is now **closed, not
pending PR 17**. `exchange_calendars` (XNYS) fixture parity is proven (see
`STATUS.md`), and the custom holiday/session arithmetic
(`_nth_weekday_of_month`, `_last_weekday_of_month`, `_easter_sunday`,
`_observed`, `_federal_holidays`, the fixed 16:00 close assumption) was
removed in place from `src/trading_research/evaluation/market_calendar.py`
...
**PR 4 update:** the hand-written EMA/RSI/MACD/TRIX/Bollinger *formulas*
are removed from `scripts/indicators.py` in PR 4 itself (fixture parity
proven, see `STATUS.md`) — that part of this row is closed, not deferred
to PR 17. **This is not a full closure of the row**, and TA-Lib is not
being claimed as the sole authority for every calculation: a thin custom
compatibility adapter is retained on top of TA-Lib's EMA/RSI/BBANDS
primitives (MACD/TRIX composition from `talib.EMA` rather than
`talib.MACD()`/`talib.TRIX()`, the flat-price RSI override, %B, and
zero-denominator handling — see `COMPONENT_MATRIX.md` and `STATUS.md` for
exactly which pieces and why). That adapter is intentionally retained
indefinitely for public-contract compatibility, not tracked as pending
further removal. The ATR risk indicator in `analysis/indicators.py` was
**not** part of this removal — its `Decimal`-based value calculation was
not already in PR 4's tracked scope and no parity tests were added for
it, so it remains a separate, later change (see `COMPONENT_MATRIX.md`).
