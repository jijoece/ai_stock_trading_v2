...
| — | **Pre-step before PR 6** | Resolve the LumiBot-backtest-mode dependency/process-boundary question in `DECISIONS.md` D4 open item 1. **IN REVIEW / BLOCKED (2026-07-26):** Opus review and pinned-version feasibility spike complete (`pre-step-06/EVALUATION.md`); `backtest_runtime/` selected; but ADR 0009 is **Proposed, not Accepted** and no reproducible install path exists yet. A first pass under Sonnet reached a different conclusion and is withdrawn | PR 5 | — | Opus review |
| 6 | LumiBot backtest evaluation adapter | New adapter beside existing `backtesting/engine.py`, in the boundary ADR 0009 defines (`backtest_runtime/`, its own distribution — **not** a second in-process import site); no deletion. Also repairs the AST import-boundary test, which currently skips in CI (ADR 0009 Decision 4) | Pre-step above **and ADR 0009 acceptance** | Medium | Sonnet, Opus review |
