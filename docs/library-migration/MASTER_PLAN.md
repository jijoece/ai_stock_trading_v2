...
| 6 | LumiBot backtest evaluation adapter | **IMPLEMENTED, NOT MERGED** (branch `migration/06-lumibot-backtest-adapter`; see `STATUS.md` "Completed work (PR 6)"). New adapter beside existing `backtesting/engine.py`, in the boundary ADR 0009 defines (`backtest_runtime/`, its own distribution — **not** a second in-process import site); no deletion. **Deliverables, all merge conditions rather than preconditions (ADR 0009 Decision 4):** create `backtest_runtime/`; an installable `backtest_runtime/pyproject.toml` pinning `lumibot==4.5.78` with an explicit `requires-python`; the distribution's own tests, with no `importorskip` guard; and a **blocking** `backtest-runtime-tests` CI job asserting `pip check`, the exact LumiBot version, and the five credential-safety properties of ADR 0009 Decision 2 (no credential value available or loaded; nothing loaded from the environment or a `.env`/`.env.local`; no broker or live data provider initialized; zero outbound attempts; determinism) — **not** zero credential reads. Also repairs the AST import-boundary test, which currently skips in CI | Pre-step above (complete) | Medium | Sonnet, Opus review |
| 7 | Backtest parity report | **IMPLEMENTED, NOT MERGED** (branch `migration/07-backtest-parity-report`; report at `docs/library-migration/pr7/PARITY_REPORT.md`, raw data under `pr7/results/`). Ran a five-case checked-in fixture set through both engines in their two separate environments (ADR 0009 Decision 5) and compared orders, fills, entry/exit timestamps, prices, quantities, cash, positions, fees, realized/unrealized P&L, equity, drawdown and final value. Comparison method: **Option A** — the narrowest legacy-engine equivalent, recorded in `DECISIONS.md` D6; `backtest_runtime` was not extended. Every difference classified, enforced by the comparator itself: 10 old-engine defect (`BacktestResult` carries no order or position records), 5 adapter defect (a fill's `market_date` is stamped one session late), 82 intentional library semantic difference, 12 unsupported requirement (fees/slippage, realized P&L). Nothing was patched — PR 8 decides | PR 6 | Medium | Sonnet |
