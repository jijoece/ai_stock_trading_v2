...
| 1 | Dependency foundation and planning corrections | `pyproject.toml` + `paper_runtime/pyproject.toml`: add `exchange_calendars` as a base dependency, `indicators`/`analytics`/`observability` extras, `hypothesis`/`time-machine` into `dev`, align LumiBot pins; Python floor stays `>=3.10` (VectorBT floor requirement does not apply — not added this PR); VectorBT and Riskfolio-Lib deferred, not added. See `DEPENDENCY_MATRIX.md` Section 6 for the full correction record | PR 0 | Low | Sonnet |
