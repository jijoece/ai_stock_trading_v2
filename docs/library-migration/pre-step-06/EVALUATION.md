...
drafted with status **Proposed** — not Accepted, pending owner approval.
...
**Finding 1 — LumiBot reads broker credentials at import, unconditionally.**
Importing `lumibot` read **277 distinct environment variables**, of which
**64 are credential-named**, including `ALPACA_API_KEY`, `ALPACA_API_SECRET`,
`ALPACA_IS_PAPER`, `TRADIER_ACCESS_TOKEN`, `IB_PASSWORD`,
...
### 2.3 Option C's dependency conflict, re-verified at 4.5.78
...
while the `jsonschema` conflict remains, and §2.3 re-confirmed that conflict at
...
## 7. What is still required before PR 6 may start

This pre-step is **not** complete. Remaining gates, tracked in `STATUS.md`:

1. **Owner approval of ADR 0009** — drafted Proposed, not Accepted. It adds a
   third distribution to the repository; that is the owner's call, not this
   review's.
2. **A reproducible CI/install path must exist** — `backtest_runtime/` does
   not exist yet. Creating it, with its blocking CI job, is PR 6 implementation
   work and is deliberately not done here.
3. The feasibility spike passes (this document) and the Opus review is
   complete (this document). Those two gates are met.
