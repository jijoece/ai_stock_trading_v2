...
| Riskfolio-Lib | Defer (PR 12, evaluated 2026-08-23) | OSI-approved and technically conflict-free (live-verified against the adopted `vectorbt==1.1.0`), but its 82-package closure has no current in-repo consumer — `COMPONENT_MATRIX.md`'s "Portfolio optimization" row lists no existing implementation to migrate off of | No portfolio-optimization capability exists; none is added |
