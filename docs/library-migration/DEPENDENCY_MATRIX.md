...
| Riskfolio-Lib | Defer (PR 12, evaluated 2026-08-23) | OSI-approved and technically conflict-free on Python >=3.11,<3.15 only (live-verified against the adopted `vectorbt==1.1.0`; the adopted `vectorbt>=1.1.0,<1.2` range cannot resolve on this repository's `>=3.10` project-wide floor without also raising it to `>=3.11`, nor on Python 3.15+ without a future VectorBT upgrade), but its 82-package closure has no current in-repo consumer — `COMPONENT_MATRIX.md`'s "Portfolio optimization" row lists no existing implementation to migrate off of | No portfolio-optimization capability exists; none is added |
