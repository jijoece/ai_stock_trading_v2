...
| Vectorized research/parameter sweeps | `src/trading_research/vector_research/adapter.py` | New capability | VectorBT 1.1.0 (Apache-2.0 + Commons Clause, owner-approved exception — `DECISIONS.md` D4) | **Adopted (PR 5)** — new `research` optional-dependency group, additive, no execution authority | New adapter (`ParameterSweepResult`/`run_parameter_sweep`), isolated `research` group requiring Python >=3.11 | No removal — new code |
