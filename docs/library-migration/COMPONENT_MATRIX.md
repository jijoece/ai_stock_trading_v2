...
| Event-driven backtesting | `backtesting/engine.py` | Evaluate (Category B) | LumiBot backtest mode | Evaluate (PR 6/7/8) — **PR 6 blocked**, pending acceptance of ADR 0009 | Adapter in its own isolated, credential-free distribution `backtest_runtime/` (ADR 0009, **Proposed**), beside the existing engine, no deletion yet. **Not** a second in-process import site — an earlier proposal to that effect is withdrawn (`DECISIONS.md` D4) | Decision gated on PR 7 parity report |
| Alpaca paper execution | `paper_runtime/.../lumibot_gateway.py`, `runtime/lumibot/adapter.py` | Already library-based | LumiBot (unchanged) | No change to import boundary; PR 1 removed the root `paper` extra (unresolvable `jsonschema` conflict, `DECISIONS.md` D5) — `paper_runtime/pyproject.toml` is the sole LumiBot dependency declaration today; if ADR 0009 is accepted, `backtest_runtime/pyproject.toml` becomes a second one, in its own separately-installed environment, with the root `pyproject.toml` still declaring none (see D5's reconciliation note) | Existing `runtime/lumibot/` boundary | No |
