...
| Technical indicators (`scripts/indicators.py`) | TA-Lib (`talib.EMA`/`RSI`/`BBANDS`, plus `talib.EMA`-composed MACD/TRIX) | Commodity | TA-Lib | **Adopted (PR 4) — TA-Lib is the sole authority** | Thin wrapper for warm-up/null/rounding parity; one intentional preserved semantic (flat-price RSI stays 100, not TA-Lib's 0) | Yes — custom EMA/RSI/MACD/TRIX/Bollinger formulas removed |
