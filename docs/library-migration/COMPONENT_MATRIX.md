...
| Event-driven backtesting | `backtesting/engine.py` | Evaluate (Category B) | LumiBot backtest mode | Evaluate (PR 6/7/8) — **PR 6 implemented, not merged** (branch `migration/06-lumibot-backtest-adapter`; ADR 0009 accepted 2026-08-01) | Adapter in its own isolated, credential-free distribution `backtest_runtime/` (ADR 0009, **Accepted**; the distribution now exists — see `STATUS.md` "Completed work (PR 6)"), beside the existing engine, no deletion yet. **Not** a second in-process import site — an earlier proposal to that effect is withdrawn (`DECISIONS.md` D4) | Decision gated on PR 7 parity report |
