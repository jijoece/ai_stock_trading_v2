...
| Event-driven backtesting | `backtesting/engine.py` | Evaluate (Category B) | LumiBot backtest mode | Evaluate (PR 6/7/8) — **PR 6 unblocked, not started** (ADR 0009 accepted 2026-08-01) | Adapter in its own isolated, credential-free distribution `backtest_runtime/` (ADR 0009, **Accepted**; the distribution is PR 6's deliverable and does not exist yet), beside the existing engine, no deletion yet. **Not** a second in-process import site — an earlier proposal to that effect is withdrawn (`DECISIONS.md` D4) | Decision gated on PR 7 parity report |
